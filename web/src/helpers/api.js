...
export let API = axios.create({
  baseURL: import.meta.env.VITE_REACT_APP_SERVER_URL
    ? import.meta.env.VITE_REACT_APP_SERVER_URL + '/llmapi'
    : '/llmapi',
  headers: {
...
export async function onDiscordOAuthClicked(client_id, options = {}) {
  ...
  if (!state) return;
  const redirect_uri = `${window.location.origin}/llmapi/oauth/discord`;
...
export async function onOIDCClicked(
  ...
  url.searchParams.set('redirect_uri', `${window.location.origin}/llmapi/oauth/oidc`);
...
export async function onCustomOAuthClicked(provider, options = {}) {
  ...
  try {
    const redirect_uri = `${window.location.origin}/llmapi/oauth/${provider.slug}`;
